...
import AvatarUploader from "../components/common/ImageUploader";
...
const ManageProduct = () => {
  ...
  const handleAddVariantRowToggle = (paintId) => {
    setShowAddVariantRow((prev) => (prev === paintId ? null : paintId));
  };

  const handleSaveVariantPaint = (paintId) => {
      ...
      if (wallpaper.id === wallpaperId) {
        ...
        if (existingVariantIndex !== -1) {
          ...
          wallpaper.variants[existingVariantIndex].quantity =
            ...
            Number(newVariant.quantity);
            wallpaper.variants[existingVariantIndex].price = Number(newVariant.price);
        } else {
          // Add new variant if it doesn’t exist
          const selectedVariant = variantAvailable.find(
            (variant) => variant.variantId === selectedVariantId
  ...
  const handleDeleteVariant = (paintId, variantId) => {
    const updatedPaints = editRow.paints.map((paint) => {
      if (paint.id === paintId) {
        return {
          ...paint,
  ...
  const handleDeleteFloor = (floorId) => {
    setEditRow((prevEditRow) => ({
      ...prevEditRow,
      floors: prevEditRow.floors.filter((floor) => floor.id !== floorId),
    }));
  };
    ...
    setEditRow((prev) => {
      ...
      newProperties.forEach((newProperty) => {
        const existingIndex = updatedProperties.findIndex((p) => {
          console.log("Checking p:", p); // Log current `p`
          console.log("Current newProperty:", newProperty); // Log current `newProperty`
    ...
    <Stack direction="row" spacing={1} my={1}>
      ...
      >
        ...
        >
          <Stack direction="column" spacing={2} width="100%" my={2}>
            ...
            >
              <TextareaAutosize
                ...
                onBlur={(e) => (e.target.parentNode.style.borderColor = "#ccc")}
              />
            </Box>
            ...
            <>
              ...
              `}
                  ...
                  <Table size="small" stickyHeader>
                    ...
                    {/* <Box
                    sx={{
                      maxHeight: "400px", // Giới hạn chiều cao của phần thân
                      overflowY: "auto",
                      "&::-webkit-scrollbar": {
                        display: "none", // Ẩn thanh cuộn cho Chrome, Safari và Edge
                      },
                      "-ms-overflow-style": "none", // Ẩn thanh cuộn cho IE và Edge Legacy
                    ...
                    <TableBody>
                      {editRow.paints.map((paint, paintIndex) => (
                        <React.Fragment key={paint.id}>
                          ...
                          {showAddVariantRow === paint.id && (
                            <TableRow
                              sx={{
                                backgroundColor:
                                  paintIndex % 2 === 0 ? "#f5f5f5" : "#ffffff", // Thay đổi màu nền cho hàng màu
                              }}
                            >
                              <TableCell
                                colSpan={7}
                              ...
                              >
                                ...
                                >
                                  {filteredVariants
                                    .filter((variant) => {
                                      // Kiểm tra nếu có Paint, Floor hoặc Wallpaper trong editRow và chỉ hiển thị các variant tương ứng
                                      if (
                                        editRow?.paints?.length > 0 &&
                                        variant.categoryName === "Paint"
                                      ) {
                                        return true; // Hiển thị các variant có category là Paint
                                      }
                                      if (
                                        editRow?.floors?.length > 0 &&
                                        variant.categoryName === "Floor"
                                      ) {
                                        return true; // Hiển thị các variant có category là Floor
                                      }
                                      if (
                                        editRow?.wallpapers?.length > 0 &&
                                        variant.categoryName === "Wallpaper"
                                      ) {
                                        return true; // Hiển thị các variant có category là Wallpaper
                                      }
                                      return false; // Nếu không có điều kiện nào phù hợp, không hiển thị variant
                                    })
                                    .map((variant) => (
                                      <MenuItem
                                        key={variant.variantId}
                                ...
                                <Button
                                  ...
                                  size="small"
                                  onClick={() =>
                                    handleSaveVariantPaint(paint.id)
                                  }
                          ...
                          >
                            <TableCell colSpan={7} align="center">
                              <Button
                                variant="outlined"
                                onClick={() =>
                                  handleAddVariantRowToggle(paint.id)
                                }
                              >
                                {showAddVariantRow === paint.id
                                  ? "Cancel"
              ...
              >
                ...
                >
                  ...
                  <Table size="small" stickyHeader>
                    ...
                    <TableBody>
                      {editRow.wallpapers.map((wallpaper, floorIndex) => (
                        <React.Fragment key={wallpaper.id}>
                          {wallpaper.variants.length > 0 ? (
                            wallpaper.variants.map((variant, variantIndex) => (
                              <TableRow
                                sx={{
                                  backgroundColor:
                                  variantIndex % 2 === 0
                          ...
                          {showAddVariantRow === wallpaper.id && (
                            <TableRow
                              sx={{
                                backgroundColor:
                                  floorIndex % 2 === 0 ? "#f5f5f5" : "#ffffff", // Thay đổi màu nền cho hàng màu
                              }}
                            >
                              <TableCell
                                colSpan={7}
                              ...
                              >
                                ...
                                >
                                  {filteredVariants.map((variant) => (
                                    <MenuItem
                                      ...
                                      value={variant.variantId}
                                    >
                                      {variant.sizeName} - {variant.packageType}{" "}
                                      - {variant.categoryName}
                                    </MenuItem>
                                  ))}
                                </TextField>
                                ...
                                <Button
                                  ...
                                  size="small"
                                  onClick={() =>
                                    handleSaveVariantWallpaper(wallpaper.id)
                                  }
                                  sx={{ minWidth: 60, height: "36px" }}
                                >
                                  Save
                          ...
                          >
                            <TableCell colSpan={7} align="center">
                              ...
                              <Button
                                variant="outlined"
                                color="secondary"
                                onClick={() => handleDeleteFloor(wallpaper.id)}
                                sx={{ ml: 1 }}
                              >
                                Delete Box
                              </Button>
                          ...
                          >
                            <TableCell colSpan={7} align="center">
                              <Button
                                variant="outlined"
                                onClick={() =>
                                  handleAddVariantRowToggle(floor.id)
                                }
                              >
                                {showAddVariantRow === floor.id
                                  ? "Cancel"
            ...
            {editRow?.images?.length > 0 ? (
              editRow.images.map((image, index) => (
                <Box
                  sx={{
                    position: "relative",
                    width: "20%", // Adjust as needed for responsive sizing
                    height: "20%",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
          ...
          <AvatarUploader handleUpload={setAvatarUrl} />
        </DialogContent>
        <DialogActions>
