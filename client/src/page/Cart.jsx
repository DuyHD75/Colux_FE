...
const Cart = () => {
  ...
  const handleCheckout = (data) => {
    ...
    const checkoutData = {
      ...
      totalAmount: data.reduce(
        (acc, product) =>
          acc + product.cartItemQuantity * product.cartItemVariant.priceSell,
        0
      ),
      billing: {},
      shippingFee: 0,
  ...
  const handleCheckboxChange = (variantId, productId, type, isChecked) => {
  ...
  const getCheckedProducts = () => {
    return products&&products.filter((product) => {
      ...
      // Xác định loại sản phẩm và ID tương ứng
      if (product.cartItemVariant.productDetails.wallpaperDetails) {
        productId =
          product.cartItemVariant.productDetails.wallpaperDetails.wallpaperId;
        type = "wallpaper";
      } else if (product.cartItemVariant.productDetails.paintDetails) {
        productId = product.cartItemVariant.productDetails.paintDetails.paintId;
        type = "paint";
      } else if (product.cartItemVariant.productDetails.floorDetails) {
        productId = product.cartItemVariant.productDetails.floorDetails.floorId;
        type = "floor";
      }

      // Kiểm tra checkbox có được đánh dấu hay không
      return checkedProducts[variantId]?.[productId]?.isChecked;
    });
  };
  ...
  const handleDecrease = (index) => {
    ...
    updatedProducts[index].cartItemQuantity = Math.max(
      1,
      updatedProducts[index].cartItemQuantity - 1
    );
    const status = 1;
    ...
    const cartItems = [
      {
        ...
        productId:
          updatedProducts[index].cartItemVariant.productDetails.productId,
        quantity: 1,
        ...(updatedProducts[index].cartItemVariant.categoryName === "Paint" && {
          paintId:
            updatedProducts[index].cartItemVariant.productDetails.paintDetails
              .paintId,
        }),
        ...(updatedProducts[index].cartItemVariant.categoryName ===
          "Wallpaper" && {
          wallpaperId:
            updatedProducts[index].cartItemVariant.productDetails
              .wallpaperDetails.wallpaperId,
        }),
        ...(updatedProducts[index].cartItemVariant.categoryName === "Floor" && {
          floorId:
            updatedProducts[index].cartItemVariant.productDetails.floorDetails
              .floorId,
        }),
      },
    ];
  ...
  const deleteCart = useCallback(async (cartId, itemDeleteRequests) => {
    const { response, err } = await cartApi.deleteCartItem(
      cartId,
      itemDeleteRequests
    );
    if (!response) {
    ...
    if (products) {
      totalAmount = products
        .filter((product) => {
          ...
          if (product.cartItemVariant.productDetails.wallpaperDetails) {
            productId =
              product.cartItemVariant.productDetails.wallpaperDetails
                .wallpaperId;
            type = "wallpaper";
          } else if (product.cartItemVariant.productDetails.paintDetails) {
            productId =
            ...
            productId =
              product.cartItemVariant.productDetails.floorDetails.floorId;
            type = "floor";
          }
      ...
      <Box p={{ xs: "73px 0 1rem 0", md: "152px 0 1rem 0" }} bgcolor="#EAEAEA">
        <Container>
          ...
          <Stack
            ...
            sx={{ padding: { xs: "0 1rem 0 1rem", lg: "0 1rem 0 0" } }}
          >
            <Box flex={8} width="100%">
              ...
              <Box
                ...
                bgcolor={{ ...backgroundConfigs.style.subBackgroundContext }}
                sx={{
                  paddingBottom: "1rem",
                  overflowY: "auto",
                  maxHeight: "800px",
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              <Typography
                                ...
                                fontSize="14px"
                              >
                                {product.cartItemVariant.priceSell}${" "}
                              </Typography>
                            </Stack>
                            <Stack
                              direction="row"
                          ...
                          <Stack
                            ...
                            alignItems="center"
                            flex={1}
                            borderLeft={{ xs: "1px solid #E5E5E5", md: 0 }}
                          >
                            <Typography
                              variant="h4"
                              sx={{
            ...
            >
              ...
              >
                ...
                >
                  Orders not picked up, received, or scheduled for delivery
              ...
              <button
                ...
                onClick={() => handleCheckout(CheckoutDetail)}
              >
                Go to checkout
              </button>
            </Box>
